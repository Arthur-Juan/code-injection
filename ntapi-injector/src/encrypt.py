# Shellcode fornecida convertida para um bytearray
shellcode = bytearray([
    
])

# Chave XOR
XOR_KEY = 0x5A

# Criptografar
encrypted_shellcode = bytearray([b ^ XOR_KEY for b in shellcode])

# Salvar em um arquivo
with open("encrypted_shellcode.bin", "wb") as f:
    f.write(encrypted_shellcode)

print("Shellcode criptografada salva em 'encrypted_shellcode.bin'")
